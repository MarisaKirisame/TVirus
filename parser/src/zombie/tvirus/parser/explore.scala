...
def pp_pat(x: Pat): String = {
  x match {
    case Pat.Var(x)    => x
    case Pat.App(f, x) => pp_pat(f) + " " + pp_pat(x)
    case _             => x.toString()
  }
}
...
def pp_expr(x: Expr): String = {
  x match {
    ...
    case Expr.App(f, x) => bracket(pp_expr(f) + " " + pp_expr(x))
    case Expr.Var(x)    => x
...
def cps_expr(x: Expr, k: Expr): Expr = {
    x match {
        case Expr.Var(x) => Expr.App(k, Expr.Var(x))
        case Expr.Abs(xs, y) => {
            val k_ = freshName
            Expr.App(k, Expr.Abs(xs ++ Seq(TBind(k_, None)), cps_expr(y, Expr.Var(k_))))
        }
        case Expr.Match(x, cases) => {
            cps_expr(x, hoas(x_ => Expr.Match(x_, cases.map((l,r) => (l, cps_expr(r, k))))))
        }
        case Expr.App(f, x) => {
